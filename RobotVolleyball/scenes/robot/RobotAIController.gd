...
func get_reward() -> float:
	...
	if robot.score > robot.other_player.score:
		reward += 100
	
	# Penalty for not hitting the ball within a certain number of steps while serving
	if steps_without_ball_hit_while_serving > 100:
		reward -= 1
